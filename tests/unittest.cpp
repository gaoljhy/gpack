...
TEST(Rule)
{
    ...
    r([&x](const Module::request&){x = 2;return "";});
...
TEST(PathRouting)
{
    ...
    Module_ROUTE(app, "/file")
    ([]{
        return "file";
    });

    Module_ROUTE(app, "/path/")
    ([]{
        return "path";
    });
...
TEST(RoutingTest)
{
    ...
    Module_ROUTE(app, "/0/<uint>")
    ([&](uint32_t b){
        B = b;
        return "OK";
    });

    Module_ROUTE(app, "/1/<int>/<uint>")
    ([&](int a, uint32_t b){
        A = a; B = b;
        return "OK";
    });

    Module_ROUTE(app, "/4/<int>/<uint>/<double>/<string>")
    ([&](int a, uint32_t b, double c, string d){
        A = a; B = b; C = c; D = d;
        return "OK";
    });

    Module_ROUTE(app, "/5/<int>/<uint>/<double>/<string>/<path>")
...
{
    SimpleApp app;
    Module_ROUTE(app, "/")([](const Module::request&, Module::response&)
    {
    });
}
...
TEST(http_method)
{
    ...
    Module_ROUTE(app, "/get_only")
        .methods("GET"_method)
    ([](const request& /*req*/){
        return "get";
    });
    Module_ROUTE(app, "/post_only")
        .methods("POST"_method)
    ([](const request& /*req*/){
        return "post";
    });
    Module_ROUTE(app, "/patch_only")
        .methods("PATCH"_method)
    ([](const request& /*req*/){
        return "patch";
    });
    Module_ROUTE(app, "/purge_only")
...
TEST(multi_server)
{
    static char buf[2048];
    SimpleApp app1, app2;
    Module_ROUTE(app1, "/").methods("GET"_method, "POST"_method)([]{return "A";});
    Module_ROUTE(app2, "/").methods("GET"_method, "POST"_method)([]{return "B";});
...
TEST(template_basic)
{
    auto t = Module::mustache::compile(R"---(attack of {{name}})---");
    Module::mustache::context ctx;
    ctx["name"] = "killer tomatoes";
    auto result = t.render(ctx);
    ASSERT_EQUAL("attack of killer tomatoes", result);
    //Module::mustache::load("basic.mustache");
}

TEST(template_load)
{
    Module::mustache::set_base(".");
    ofstream("test.mustache") << R"---(attack of {{name}})---";
    auto t = Module::mustache::load("test.mustache");
    Module::mustache::context ctx;
    ctx["name"] = "killer tomatoes";
    auto result = t.render(ctx);
    ASSERT_EQUAL("attack of killer tomatoes", result);
...
TEST(middleware_simple)
{
    ...
    Module_ROUTE(app, "/")([&](const Module::request& req)
    {
...
TEST(middleware_context)
{
    ...
    int x{};
    Module_ROUTE(app, "/")([&](const request& req){
        {
    ...
    Module_ROUTE(app, "/break")([&](const request& req){
        {
            auto& ctx = app.get_context<FirstMW>(req);
            ctx.v.push_back("handle");
        }
...
TEST(simple_url_params)
{
    ...
    Module_ROUTE(app, "/params")
    ([&last_url_params](const Module::request& req){
