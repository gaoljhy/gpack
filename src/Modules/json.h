...
//#define Module_JSON_NO_ERROR_CHECK
...
#define Module_json_likely(x) __builtin_expect(x, 1)
#define Module_json_unlikely(x) __builtin_expect(x, 0)
#else
#define Module_json_likely(x) x
#define Module_json_unlikely(x) x
#endif
    ...
    namespace json
    {
        ...
        namespace detail 
        {
            ...
            private:
                ...
                friend rvalue Module::json::load(const char* data, size_t size);
            };
        ...
        public:
            ...
            type t() const
            {
#ifndef Module_JSON_NO_ERROR_CHECK
        ...
        inline rvalue load_nocopy_internal(char* data, size_t size)
        {
            ...
            struct Parser
            {
                ...
                bool consume(char c)
                {
                    if (Module_json_unlikely(*data != c))
                        return false;
                    data++;
                    return true;
                }
                ...
                {
                    if (Module_json_unlikely(!consume('"')))
                        return {};
                    char* start = data;
                    uint8_t has_escaping = 0;
                    while(1)
                    {
                        if (Module_json_likely(*data != '"' && *data != '\\' && *data != '\0'))
                        {
                            data ++;
                        }
                        else if (*data == '"')
                ...
                rvalue decode_number()
                {
                    ...
                    while(Module_json_likely(state != Invalid))
                    {
                        switch(*data)
                        {
                            case '0':
                            ...
                            case 'e': case 'E':
                                ...
                                break;
                            default:
                                if (Module_json_likely(state == NumberParsingState::ZeroFirst || 
                                        state == NumberParsingState::Digits || 
                                        state == NumberParsingState::DigitsAfterPoints || 
                                        state == NumberParsingState::DigitsAfterE))
        ...
        class wvalue
        {
            friend class Module::mustache::template_t;
        public:
...
#undef Module_json_likely
#undef Module_json_unlikely
