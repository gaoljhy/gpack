...
#include "Module/http_response.h"
#include "Module/http_request.h"
#include "Module/utility.h"
#include "Module/logging.h"
#include "Module/websocket.h"
...
namespace Module
{
    ...
    namespace detail
    {
        namespace routing_handler_call_helper
        {
            ...
            struct Wrapped
            {
                ...
                {
                    using type = std::function<void(const Module::request&, Module::response&, Args...)>;
    ...
    public:
        ...
        {
            new Module::websocket::Connection<SocketAdaptor>(req, std::move(adaptor), open_handler_, message_handler_, close_handler_, error_handler_, accept_handler_);
        }
#ifdef Module_ENABLE_SSL
        void handle_upgrade(const request& req, response&, SSLAdaptor&& adaptor) override
        {
            new Module::websocket::Connection<SSLAdaptor>(req, std::move(adaptor), open_handler_, message_handler_, close_handler_, error_handler_, accept_handler_);
        }
#endif
    ...
    protected:
        std::function<void(Module::websocket::connection&)> open_handler_;
        std::function<void(Module::websocket::connection&, const std::string&, bool)> message_handler_;
        std::function<void(Module::websocket::connection&, const std::string&)> close_handler_;
        std::function<void(Module::websocket::connection&)> error_handler_;
        std::function<bool(const Module::request&)> accept_handler_;
    };
    ...
    public:
        ...
        operator()(Func&& f)
        {
            static_assert(black_magic::CallHelper<Func, black_magic::S<Args...>>::value ||
                black_magic::CallHelper<Func, black_magic::S<Module::request, Args...>>::value , 
                "Handler type is mismatched with URL parameters");
            static_assert(!std::is_same<void, decltype(f(std::declval<Args>()...))>::value, 
                "Handler function cannot have void return type; valid return types: string, int, Module::resposne, Module::json::wvalue");
        ...
        operator()(Func&& f)
        {
            static_assert(black_magic::CallHelper<Func, black_magic::S<Args...>>::value ||
                black_magic::CallHelper<Func, black_magic::S<Module::request, Args...>>::value, 
                "Handler type is mismatched with URL parameters");
            static_assert(!std::is_same<void, decltype(f(std::declval<Module::request>(), std::declval<Args>()...))>::value, 
                "Handler function cannot have void return type; valid return types: string, int, Module::resposne, Module::json::wvalue");
            ...
            handler_ = (
                ...
                [f]
#endif
                (const Module::request& req, Module::response& res, Args ... args){
                    res = response(f(req, args...));
                    res.end();
                });
        }
        ...
        operator()(Func&& f)
        {
            static_assert(black_magic::CallHelper<Func, black_magic::S<Args...>>::value ||
                black_magic::CallHelper<Func, black_magic::S<Module::request, Args...>>::value ||
                black_magic::CallHelper<Func, black_magic::S<Module::request, Module::response&, Args...>>::value
                , 
                "Handler type is mismatched with URL parameters");
            static_assert(std::is_same<void, decltype(f(std::declval<Module::request>(), std::declval<Module::response&>(), std::declval<Args>()...))>::value, 
    ...
    private:
        std::function<void(const Module::request&, Module::response&, Args...)> handler_;
...
public:
    ...
    private:
        void debug_node_print(Node* n, int level)
        {
            for(int i = 0; i < (int)ParamType::MAX; i ++)
            {
                ...
                {
                    Module_LOG_DEBUG << std::string(2*level, ' ') /*<< "("<<n->param_childrens[i]<<") "*/;
                    switch((ParamType)i)
                    {
                        case ParamType::INT:
                            Module_LOG_DEBUG << "<int>";
                            break;
                        case ParamType::UINT:
                            Module_LOG_DEBUG << "<uint>";
                            break;
                        case ParamType::DOUBLE:
                            Module_LOG_DEBUG << "<float>";
                            break;
                        case ParamType::STRING:
                            Module_LOG_DEBUG << "<str>";
                            break;
                        case ParamType::PATH:
                            Module_LOG_DEBUG << "<path>";
                            break;
                        default:
                            Module_LOG_DEBUG << "<ERROR>";
                            break;
                    }
            ...
            {
                Module_LOG_DEBUG << std::string(2*level, ' ') /*<< "(" << kv.second << ") "*/ << kv.first;
    ...
    public:
        ...
        void handle_upgrade(const request& req, response& res, Adaptor&& adaptor)
        {
            ...
            if (!rule_index)
            {
                Module_LOG_DEBUG << "Cannot match rules " << req.url << ' ' << method_name(req.method);
                res = response(404);
                res.end();
                return;
            }
            ...
            {
                Module_LOG_INFO << "Redirecting to a url with trailing slash: " << req.url;
                res = response(301);
            ...
            Module_LOG_DEBUG << "Matched rule (upgrade) '" << rules[rule_index]->rule_ << "' " << (uint32_t)req.method << " / " << rules[rule_index]->get_methods();
            ...
            {
                Module_LOG_ERROR << "An uncaught exception occurred: " << e.what();
                res = response(500);
                res.end();
                return;   
            }
            ...
            {
                Module_LOG_ERROR << "An uncaught exception occurred. The type was unknown so no information was available.";
                res = response(500);
                res.end();
                return;   
            }
        ...
        void handle(const request& req, response& res)
        {
            ...
            Module_LOG_DEBUG << "Matched rule '" << rules[rule_index]->rule_ << "' " << (uint32_t)req.method << " / " << rules[rule_index]->get_methods();
        ...
        void debug_print()
        {
            ...
            {
                Module_LOG_DEBUG << method_name((HTTPMethod)i);
