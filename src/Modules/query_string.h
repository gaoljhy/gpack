...
#define Module_QS_ISHEX(x)    ((((x)>='0'&&(x)<='9') || ((x)>='A'&&(x)<='F') || ((x)>='a'&&(x)<='f')) ? 1 : 0)
#define Module_QS_HEX2DEC(x)  (((x)>='0'&&(x)<='9') ? (x)-48 : ((x)>='A'&&(x)<='F') ? (x)-55 : ((x)>='a'&&(x)<='f') ? (x)-87 : 0)
#define Module_QS_ISQSCHR(x) ((((x)=='=')||((x)=='#')||((x)=='&')||((x)=='\0')) ? 0 : 1)
...
inline int qs_strncmp(const char * s, const char * qs, size_t n)
{
    ...
    while(n-- > 0)
    {
        ...
        if ( ! Module_QS_ISQSCHR(u1) ) {  u1 = '\0';  }
        if ( ! Module_QS_ISQSCHR(u2) ) {  u2 = '\0';  }
        ...
        if ( u1 == '%' ) // easier/safer than scanf
        {
            ...
            if ( Module_QS_ISHEX(unyb) && Module_QS_ISHEX(lnyb) )
                u1 = (Module_QS_HEX2DEC(unyb) * 16) + Module_QS_HEX2DEC(lnyb);
            else
                u1 = '\0';
        }
        ...
        if ( u2 == '%' ) // easier/safer than scanf
        {
            ...
            if ( Module_QS_ISHEX(unyb) && Module_QS_ISHEX(lnyb) )
                u2 = (Module_QS_HEX2DEC(unyb) * 16) + Module_QS_HEX2DEC(lnyb);
            else
                u2 = '\0';
        }
...
inline int qs_decode(char * qs)
{
    ...
    while( Module_QS_ISQSCHR(qs[j]) )
    {
        if ( qs[j] == '+' ) {  qs[i] = ' ';  }
        else if ( qs[j] == '%' ) // easier/safer than scanf
        {
            if ( ! Module_QS_ISHEX(qs[j+1]) || ! Module_QS_ISHEX(qs[j+2]) )
            {
                qs[i] = '\0';
                return i;
            }
            qs[i] = (Module_QS_HEX2DEC(qs[j+1]) * 16) + Module_QS_HEX2DEC(qs[j+2]);
            j+=2;
        }
        else
        {
