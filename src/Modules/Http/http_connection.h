...
#include "Module/http_parser_merged.h"

#include "Module/parser.h"
#include "Module/http_response.h"
#include "Module/logging.h"
#include "Module/settings.h"
#include "Module/dumb_timer_queue.h"
#include "Module/middleware_context.h"
#include "Module/socket_adaptors.h"
...
namespace Module
{
    ...
    public:
...
#ifdef Module_ENABLE_DEBUG
            connectionCount ++;
            Module_LOG_DEBUG << "Connection open, total " << connectionCount << ", " << this;
#endif
        }
...
#ifdef Module_ENABLE_DEBUG
            connectionCount --;
            Module_LOG_DEBUG << "Connection closed, total " << connectionCount << ", " << this;
#endif
        }
        ...
        void handle()
        {
            ...
            Module_LOG_INFO << "Request: " << boost::lexical_cast<std::string>(adaptor_.remote_endpoint()) << " " << this << " HTTP/" << parser_.http_major << "." << parser_.http_minor << ' '
        ...
        {
            Module_LOG_INFO << "Response: " << this << ' ' << req_.raw_url << ' ' << res.code << ' ' << close_connection_;
            ...
            if (!adaptor_.is_open())
            {
                //Module_LOG_DEBUG << this << " delete (socket is closed) " << is_reading << ' ' << is_writing;
                //delete this;
                return;
            }
    ...
    private:
            ...
            adaptor_.socket().async_read_some(boost::asio::buffer(buffer_), 
                [this](const boost::system::error_code& ec, std::size_t bytes_transferred)
                {
                    ...
                    if (error_while_reading)
                    {
                        ...
                        adaptor_.close();
                        is_reading = false;
                        Module_LOG_DEBUG << this << " from read(1)";
                        check_destroy();
                    }
            ...
            boost::asio::async_write(adaptor_.socket(), buffers_, 
                [&](const boost::system::error_code& ec, std::size_t /*bytes_transferred*/)
                {
                    ...
                    if (!ec)
                    {
                        ...
                        {
                            adaptor_.close();
                            Module_LOG_DEBUG << this << " from write(1)";
                            check_destroy();
                        }
                    }
                    else
                    {
                        Module_LOG_DEBUG << this << " from write(2)";
                        check_destroy();
                    }
                });
        }
        ...
        void check_destroy()
        {
            Module_LOG_DEBUG << this << " is_reading " << is_reading << " is_writing " << is_writing;
            if (!is_reading && !is_writing)
            {
                Module_LOG_DEBUG << this << " delete (idle) ";
                delete this;
            }
        }
        ...
        {
            Module_LOG_DEBUG << this << " timer cancelled: " << timer_cancel_key_.first << ' ' << timer_cancel_key_.second;
        ...
        void start_deadline(/*int timeout = 5*/)
        {
            ...
            Module_LOG_DEBUG << this << " timer added: " << timer_cancel_key_.first << ' ' << timer_cancel_key_.second;
        }
