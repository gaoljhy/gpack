...
#include "Module/http_connection.h"
...
namespace Module
{
    ...
    Server(Handler* handler, std::string bindaddr, uint16_t port, std::tuple<Middlewares...>* middlewares = nullptr, uint16_t concurrency = 1, typename Adaptor::context* adaptor_ctx = nullptr)
        ...
        void run()
        {
                ...
                v.push_back(
                        std::async(std::launch::async, [this, i, &init_count]{
                            ...
                            while(1)
                            {
                                ...
                                {
                                    Module_LOG_ERROR << "Worker Crash: An uncaught exception occurred: " << e.what();
                                }
                            }
                        }));
            ...
            Module_LOG_INFO << server_name_ << " server is running at " << bindaddr_ <<":" << port_
                          << " using " << concurrency_ << " threads";
            Module_LOG_INFO << "Call `app.loglevel(Module::LogLevel::Warning)` to hide Info level logs.";
    ...
    private:
        ...
        std::string server_name_ = "Module/0.1";
        uint16_t port_;
